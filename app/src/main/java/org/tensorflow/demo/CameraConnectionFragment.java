  ...
  private static Size chooseOptimalSize(
    ...
    for (final Size option : choices) {
      if (option.getHeight() >= MINIMUM_PREVIEW_SIZE && option.getWidth() >= MINIMUM_PREVIEW_SIZE) {
        // Quick fix to inverted height and width.
        if (option.getHeight() < option.getWidth()) {
          // Invert height and width.
          LOGGER.i("Flipping height-width. Adding size: " + option.getHeight() + "x" + option.getWidth());
          Size currentOption = new Size(option.getHeight(), option.getWidth());
          bigEnough.add(currentOption);
        }
        else {
    ...
    // TensorFlow creators said: Pick the smallest of those, assuming we found any
    if (bigEnough.size() > 0) {
...
//        tfPreviewListener.takePic();
//        Toast toast = Toast.makeText(getActivity(), "Picture taken.", Toast.LENGTH_SHORT);
//        toast.show();
//      }
//    });
    return v;
  ...
  private void setUpCameraOutputs(final int width, final int height) {
    ...
    try {
      for (final String cameraId : manager.getCameraIdList()) {
        ...
        // TODO: Map is giving out bad sizes for Pixel C.
        ...
        LOGGER.i("Sensor Orientation " + sensorOrientation.toString(), "");
        ...
        previewSize =
                chooseOptimalSize(mapSize, width, height, largest);
...
//        } else {
//          LOGGER.i("Orientation_not_landscape", "");
